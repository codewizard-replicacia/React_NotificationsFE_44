...
const EditBookingForm = () => {
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_BOOKING}(${formData.BookingId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated Booking', {
        variant: 'success',
      });
      navigate({ pathname: '/bookings' });
    } else {
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/bookings' })}
              >
                Cancel
              </Button>
            </div>
